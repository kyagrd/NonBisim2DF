...
We study a problem of providing a certificate for two non-bisimilar processes,
assuming we already have a procedure for testing bisimilarity. The problem
with such an assumption is realistic within various tools [TODO cite]
...
\paragraph{Our contribution} TODO


\paragraph{Our approach} requires the bisimulation test to produce
...
so that we can extract a \emph{distinguishing formula} from that graph
when the test fails.
By definition of the bisim-set, it is natural to assume that bisimulation
testing procedures should be able produce a bisimulation set ($R$), which is
a set of process state pairs ($R \subset S\times S$) when the test succeeds.
...
known to be adequate for characterizing bisimilar processes in
a labeled transition system.

\paragraph{Our method} TODO
and 
two processes are
bisimulation testing procedure

states are represented as terms in process calculi (e.g., $\pi$-calculus)
that evolve by tacking next possible action rather than transitions over
predetermined set of process states as in LTS-style presentation.

%% Our Method (tools we use)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%% below is from llncs.tex template %%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
